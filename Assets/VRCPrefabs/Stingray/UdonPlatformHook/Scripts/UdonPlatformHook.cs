	...
	public class UdonPlatformHook : UdonSharp.UdonSharpBehaviour
{
	...
	public bool custom_force_unhook = false;
	bool menuOpen;
	...
	{	set
		{	isHooked = value;
			if (isHooked && !custom_force_unhook) // isHooked=true functions
			{
		...
		{	
			...
			if (isHooked && inheritVelocity && !custom_force_unhook) // Average the last X frames of the players global velocity.
			{
		...
		void LateUpdateFunctions() 
		{
			if (!VRC.SDKBase.Utilities.IsValid(localPlayer)) { return; }
#if !UNITY_EDITOR
            if (isHooked && !custom_force_unhook) // Count the number of InterntalUI colliders as a means to know if the menu is open or not.
			{
			...
			if (isHooked && !custom_force_unhook && !menuOpen) // Move the parented hook to the Players position
			{
				hookLastPos = hook.position;
				hook.position = localPlayer.GetPosition();
				hookOffsetPos = hook.position - hookLastPos;
			}
			if (isHooked && !custom_force_unhook && menuOpen) { localPlayer.SetVelocity(Vector3.zero); } // Override Player Velocity to make it easier to use their menu.
			...
			if (isHooked && !custom_force_unhook && !menuOpen)
			{
            ...
            if (isHooked && !custom_force_unhook)
			{
			...
			if(reduceIKDrift && isHooked && !custom_force_unhook)
			{
